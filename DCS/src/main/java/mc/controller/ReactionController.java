...
public class ReactionController {
    ...
    public String update(Model model,@ModelAttribute ReaForm form) {
		...
		ReactionForm reaction = reactionFormService.findRf(form.getId());
		if(null == reaction) {
	...
	@RequestMapping(value = "/reaction/delete",method = RequestMethod.GET)
    public String delete(Model model,int patientId,int recId) {
		...
		ReactionForm reaction = reactionFormService.findRf(recId);
		if(null == reaction) {
			model.addAttribute("error","ReactionForm not exists");
			return "404";
		}
		reactionFormService.delete(recId);
		return "redirect:/myReactions?uid="+patient.getId();
    }
